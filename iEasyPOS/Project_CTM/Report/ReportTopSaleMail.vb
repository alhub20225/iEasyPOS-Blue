Public Class ReportTopSaleMail
    ...
    Private Sub ReportDaysMail_BeforePrint(sender As Object, e As Printing.PrintEventArgs) Handles MyBase.BeforePrint
        ...
        Dim DataItemSale As DataTable = executesql("SELECT SaleItem.ItemId,SaleItem.CategoryId,Item.ItemCode,Item.ItemName,SaleItem.InitPrice,SaleItem.UnitPrice,
                                                   ...
                                                   BETWEEN '" & DTS & "' AND '" & DTE & "' AND Sale.Active= 1 and SaleItem.Active = 1
    ...
    Private Sub txt_gloss_sale_BeforePrint(sender As Object, e As Printing.PrintEventArgs) Handles txt_gloss_sale.BeforePrint
        Dim Stt As DataTable = executesql("select SUM(sale.Total) AS Total from Sale WHERE Sale.SaleDate BETWEEN '" & DTS & "' AND '" & DTE & "' AND Sale.Active=1")
        If IsDBNull(Stt(0)("Total")) Then
            Total = 0
        Else
            Total = Stt(0)("Total")
        End If
        Dim sum_withDraw As DataTable = executesql("select sum(WithdrawAmount) as WithdrawAmount from WithdrawMoney where InsertDate BETWEEN '" & DTS & "' AND '" & DTE & "'")
    ...
    Private Sub sum_spe_dc_BeforePrint(sender As Object, e As Printing.PrintEventArgs) Handles sum_spe_dc.BeforePrint
        Dim AmountDC As DataTable = executesql("Select Sum(Sale.AmountDC) As AmountDC FROM SALE WHERE Sale.SaleDate  BETWEEN '" & DTS & "' AND '" & DTE & "' AND Sale.Active= 1")
        If IsDBNull(AmountDC(0)("AmountDC")) Then
            AmtDC = 0
            Exit Sub
        Else
            Dim SelectDC As DataTable = executesql("select SUM(SaleItem.PercentDC + SaleItem.BahtDC) AS DC from SaleItem inner join Sale on(Sale.SaleId = SaleItem.SaleId) inner join PaymentDetail on (SaleItem.Saleid = PaymentDetail.SaleId) WHERE SaleItem.InsertDate BETWEEN  '" & DTS & "' AND '" & DTE & "' AND Sale.Active= 1")
    ...
    Private Sub TxtStartDate_BeforePrint(sender As Object, e As Printing.PrintEventArgs) Handles TxtStartDate.BeforePrint
        TxtStartDate.Text = Convert.ToDateTime(DTS).ToString("dd MMMM yyyy", New System.Globalization.CultureInfo("th-TH"))
    End Sub

    Private Sub TxtEndDate_BeforePrint(sender As Object, e As Printing.PrintEventArgs) Handles TxtEndDate.BeforePrint
        TxtEndDate.Text = Convert.ToDateTime(DTE).ToString("dd MMMM yyyy", New System.Globalization.CultureInfo("th-TH"))
    End Sub
