...
Where PromotionItem.ItemId='" & ItemId & "' And PromotionItem.Active ='1'")
                    ...
                    Dim selectPointRate As DataTable = executesql("Select PointRate.* From PointRate 
inner join ItemPoint on(PointRate.PointRateId = ItemPoint.PointRateId)
Where ItemPoint.ItemId = '" & ItemId & "' And ItemPoint.Active='1'")
            ...
            'Dim selectPayMentType As DataTable = executesql("SELECT PaymentId FROM PaymentType WHERE PaymentName='" & Pos_payment.PaymentType & "'")

            'Dim selectPayMentType As DataTable = executesql("SELECT PaymentId,PaymentCode FROM PaymentType WHERE PaymentName='" & Pos_payment.PaymentType & "'")
            ...
            'executesql("INSERT INTO SalePayment(SaleId,PaymentId,PaymentCode,PaidAmount,Rate,InsertDate,RefNumber,CC_Type,CC_Number) VALUES('" & selectSaleId(0)("SaleId") & "','" & selectPayMentType(0)("PaymentId") & "','" & paymentTypeCode & "','" & PaidAmount & "','" & Rate & "','" & InsertDate & "','" & RefNumber & "','" & CC_Type & "','" & CC_Number & "')")
...
Where ItemPoint.ItemId = '" & ItemId & "' And ItemPoint.Active='1'")
            ...
            'executesql("INSERT INTO SalePayment(SaleId,PaymentId,PaymentCode,PaidAmount,Rate,InsertDate,RefNumber,CC_Type,CC_Number) VALUES('" & GetSaleId & "','" & selectPayMentType(0)("PaymentId") & "','" & paymentTypeCode & "','" & PaidAmount & "','" & Rate & "','" & InsertDate & "','" & RefNumber & "','" & CC_Type & "','" & CC_Number & "')")

            'Dim BankId As Integer = BankId2
            'Dim EdcBankId As Integer = 0
            'Dim BankCardTypeId As Integer = BankCardTypeId2
            'Dim CardTypeId As Integer = CardTypeId2
            'Dim PaidAmount2 As Decimal = Pos_payment.txtPayTotal.Text
            'Dim CouponNumber As String = coupon
            'Dim VoucherNumber As String = voucher
            'Dim BankNumber As String = BankNumber2
        ...
        'If Pos_payment.PaymentType = "บัตรเงินสด" Then
        '    Exit Sub
        'End If
        ''If Pos_payment.PaymentType = "บัตรเครดิต" Then
        ''    Exit Sub
        ''End If
        'If Pos_payment.PaymentType = "คูปอง" Then
        '    Exit Sub
        'End If
        'If Pos_payment.PaymentType = "Voucher" Then
        '    Exit Sub
        'End If
        If change <> 0 Then
    ...
    Sub AddPaymentToDGCreditCard()
        'Pos_payment.DataGridViewSale.Rows.Clear()
        If Pos_Payment_CreditCard.RadDropDownBankType.Text = "" Then
            BankId2 = 0
        Else
    ...
    Sub checkVoucher()
        ...
        If checkVoucher.Rows.Count = 0 Then
            ...
            Exit Sub
        Else
            Dim dateNow As String = Convert.ToDateTime(Now).ToString("yyyy-MM-dd HH:mm:ss", New System.Globalization.CultureInfo("th-TH"))
    ...
    End Sub
    Sub loadBankType()
        Pos_Payment_CreditCard.RadDropDownCardType.Items.Clear()
        'Dim BankCardType As DataTable = ModuleGetDataAPI.gett("http://testnoi.hiveup.co/api/vbapi.php?action=SELECT&field=BankCardTypeName&table=BankCardType")
        ...
        For i = 0 To BankCardType.Rows.Count - 1
            Pos_Payment_CreditCard.RadDropDownCardType.Items.Add(BankCardType(i)("BankCardTypeName"))
        Next
    End Sub
    Sub loadBank()
        Pos_Payment_CreditCard.RadDropDownBankType.Items.Clear()
        'Dim BankName As DataTable = ModuleGetDataAPI.gett("http://testnoi.hiveup.co/api/vbapi.php?action=SELECT&field=BankName&table=Bank")
        ...
        For i = 0 To BankName.Rows.Count - 1
            Pos_Payment_CreditCard.RadDropDownBankType.Items.Add(BankName(i)("BankName"))
        Next
    End Sub
End Module
