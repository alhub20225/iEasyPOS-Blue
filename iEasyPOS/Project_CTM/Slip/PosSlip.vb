...
Public Class PosSlip
    ...
    Private Sub PosSlip_BeforePrint(sender As Object, e As PrintEventArgs) Handles MyBase.BeforePrint
        Try
            ...
            'MsgBox(GetSaleId)
            Dim Datatable1 As DataTable = executesql("SELECT SaleId,SaleDate,SaleNo,Total,SubTotal,Amountdc,partnerTxnUid,CustomerId FROM Sale WHERE SaleId=" & GetSaleId & "")

            Dim Datatable2 As DataTable = executesql("SELECT ItemCode,ItemName,UnitPrice,Qty,Total,SubTotal,InitPrice,Tax,PercentDC,BahtDC,AmtDC FROM SaleItem WHERE SaleId=" & GetSaleId & "")

            Dim Datatable3 As DataTable = executesql("SELECT PaymentDetail.SaleId,PaymentDetail.PaidAmount,PaymentType.PaymentName,PaymentType.PaymentId FROM PaymentDetail INNER JOIN PaymentType ON (PaymentType.PaymentId = PaymentDetail.PaymentTypeId) WHERE PaymentDetail.SaleId=" & GetSaleId & "")
            ...
            Dim DT_PayCard As DataTable = executesql("SELECT PaidAmount FROM PaymentDetail INNER JOIN PaymentType ON (PaymentType.PaymentId = PaymentDetail.PaymentTypeId)WHERE PaymentDetail.SaleId=" & GetSaleId & " AND PaymentDetail.PaymentTypeId = 11")
            If DT_PayCard.Rows.Count = 0 Then
                Row_CreditCard.Visible = False
            Else
                PayCard = DT_PayCard(0)("PaidAmount")
                txt_PayCredit.Text = FormatNumber(PayCard, 2)
            End If

            Dim DT_PayCash As DataTable = executesql("SELECT PaidAmount FROM PaymentDetail INNER JOIN PaymentType ON (PaymentType.PaymentId = PaymentDetail.PaymentTypeId)WHERE PaymentDetail.SaleId=" & GetSaleId & " AND PaymentDetail.PaymentTypeId = 9")
            If DT_PayCash.Rows.Count = 0 Then
                Row_Payment.Visible = False
                Row_Change.Visible = False
            Else
                PayCash = DT_PayCash(0)("PaidAmount")
                txtPay.Text = FormatNumber(PayCash, 2)
                txtChange.Text = FormatNumber((PayCash - (Datatable1(0)("SubTotal") - PayCard)), 2)
            End If
            Dim Dt As DataTable = executesql("SELECT PaymentType.PaymentName,PaymentType.PaymentId,Sale.TerminalName FROM PaymentDetail INNER JOIN PaymentType ON (PaymentType.PaymentId = PaymentDetail.PaymentTypeId) INNER JOIN Sale on (Sale.SaleId = PaymentDetail.SaleId) WHERE PaymentDetail.SaleId=" & GetSaleId & "")
    ...
    Private Sub Row_Change_BeforePrint(sender As Object, e As PrintEventArgs) Handles Row_Change.BeforePrint
    ...
    '    txtPayName.Text = "ชำระด้วย " + txtPayName.Text
    'End Sub
    ...
    Private Sub XrLabel9_BeforePrint(sender As Object, e As PrintEventArgs) Handles XrLabel9.BeforePrint
        Dim sts As DataTable = executesql("SELECT Active FROM Sale WHERE SaleId=" & GetSaleId & "")
    ...
    Private Sub txtInnit_BeforePrint(sender As Object, e As PrintEventArgs) Handles txtInnit.BeforePrint
        Dim selectTaxData As DataTable = executesql("SELECT sum(SubTotal - Tax) As Tax From SaleItem WHERE SaleId=" & GetSaleId & "")
