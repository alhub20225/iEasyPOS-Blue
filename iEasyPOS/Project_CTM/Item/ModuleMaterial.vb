...
Module ModuleMaterial
    ...
    Dim where As String
    Sub LoadComboboxVat()
        Material.ComboBoxVat.Items.Clear()
        ...
        For i = 0 To RowsDataVat - 1
            Material.ComboBoxVat.Items.Add(DataVat(i)("TaxName"))
        Next
    End Sub
    Sub loadCombobox()
        ...
        For i As Integer = 0 To selectCategory.Rows.Count - 1
            Material.ComboBoxMaterialCategory.Items.Add(selectCategory(i)("CategoryName"))
        Next
        ...
        For a As Integer = 0 To selectUint.Rows.Count - 1
            Material.ComboBoxPOUnit.Items.Add(selectUint(a)("ItemUnitName"))
            Material.ComboBoxUseUnit.Items.Add(selectUint(a)("ItemUnitName"))
            Material.ComboBoxPOUnit2.Items.Add(selectUint(a)("ItemUnitName"))
        Next
        Material.ComboBoxStatus.Controls.Clear()
        Dim statusData() As String = {"พร้อมขาย", "ไม่พร้อมขาย"}
        For r As Integer = 0 To 1
            Material.ComboBoxStatus.Items.Add(statusData(r))
        Next
    End Sub
    Sub loadData()
        clear()
        ...
        ElseIf selectCategory.Rows.Count = 0 Then
            Exit Sub
        End If
        Material.ComboBoxCateSearch.Items.Clear()
        For i As Integer = 0 To selectCategory.Rows.Count - 1
            Material.ComboBoxCateSearch.Items.Add(selectCategory(i)("CategoryName"))
        Next
    ...
    Sub loadDataGrid()
        ...
        For i As Integer = 0 To showData.Rows.Count - 1
            'field = "Qty"
            'where = "ItemId='" & showData(i)("ItemId") & "'"
            'table = "UnitInStock"
            'Dim selectQty As DataTable = ModuleGetDataAPI.gett("http://testnoi.hiveup.co/api/vbapi.php?action=SELECT&field=" & field & "&table=" & table & "&where=" & where & "")
    ...
    Sub btnSearch()
        ...
        If Material.ComboBoxCateSearch.Text = "" And Material.txtSearch.Text = "" Then
            'Dim showData As DataTable = ModuleGetDataAPI.gett("http://testnoi.hiveup.co/api/vbapi.php?action=SELECT&field=Item.ItemId,Item.ItemCode,Item.ItemName,CategoryItem.CategoryName&table=Item&join=INNER JOIN CategoryItem ON CategoryItem.CategoryId = Item.CategoryId&where=Item.Active=1 AND CategoryItem.CateGroupId=2")
        ...
        ElseIf Material.ComboBoxCateSearch.Text = "" And Material.txtSearch.Text <> "" Then
            ...
            Exit Sub
        ElseIf Material.ComboBoxCateSearch.Text <> "" And Material.txtSearch.Text = "" Then
            'Dim getId As DataTable = ModuleGetDataAPI.gett("http://testnoi.hiveup.co/api/vbapi.php?action=SELECT&field=CategoryId&table=CategoryItem&where=CategoryName='" & Material.ComboBoxCateSearch.Text & "'")
            Dim getId As DataTable = executesql("SELECT CategoryId FROM CategoryItem WHERE CategoryName='" & Material.ComboBoxCateSearch.Text & "'")
            'Dim showData As DataTable = ModuleGetDataAPI.gett("http://testnoi.hiveup.co/api/vbapi.php?action=SELECT&field=Item.ItemId,Item.ItemCode,Item.ItemName,CategoryItem.CategoryName&table=Item&join=INNER JOIN CategoryItem ON CategoryItem.CategoryId = Item.CategoryId&where=Item.Active=1 AND CategoryItem.CategoryId='" & getId(0)("CategoryId") & "' AND CategoryItem.CateGroupId=2")
            ...
            For i As Integer = 0 To showData.Rows.Count - 1
                Dim Qty As Decimal
                'Dim selectQty As DataTable = ModuleGetDataAPI.gett("http://testnoi.hiveup.co/api/vbapi.php?action=SELECT&field=Qty&table=UnitInStock&where=ItemId='" & showData(i)("ItemId") & "'")
                Dim selectQty As DataTable = executesql("Select Qty From UnitInStock Where ItemId='" & showData(i)("ItemId") & "'")
                If selectQty.Rows.Count = 0 Then
                    Qty = 0
                Else
        ...
        Else
            'Dim getId As DataTable = ModuleGetDataAPI.gett("http://testnoi.hiveup.co/api/vbapi.php?action=SELECT&field=CategoryId&table=CategoryItem&where=CategoryName='" & Material.ComboBoxCateSearch.Text & "'")
            Dim getId As DataTable = executesql("SELECT CategoryId FROM CategoryItem WHERE CategoryName='" & Material.ComboBoxCateSearch.Text & "'")
            'Dim showData As DataTable = ModuleGetDataAPI.gett("http://testnoi.hiveup.co/api/vbapi.php?action=SELECT&field=Item.ItemId,Item.ItemCode,Item.ItemName,CategoryItem.CategoryName&table=Item&join=INNER JOIN CategoryItem ON CategoryItem.CategoryId = Item.CategoryId&where=Item.Active=1 AND CategoryItem.CategoryId='" & getId(0)("CategoryId") & "'  AND Item.ItemName LIKE '%" & Material.txtSearch.Text & "%'  AND CategoryItem.CateGroupId=2")
    ...
    Sub showData()
        ...
        'Dim selectPOUnitName As DataTable = ModuleGetDataAPI.gett("http://testnoi.hiveup.co/api/vbapi.php?action=SELECT&field=ItemUnitName&table=ItemUnit&where=ItemUnitId='" & selectItem(0)("POUnitId") & "'")
        ...
        'Dim selectPOUnitName2 As DataTable = ModuleGetDataAPI.gett("http://testnoi.hiveup.co/api/vbapi.php?action=SELECT&field=ItemUnitName&table=ItemUnit&where=ItemUnitId='" & selectItem(0)("POUnitId2") & "'")
    ...
    Sub clear()
        ...
        Material.PictureBoxResize.BackgroundImage = Nothing
        Material.ComboBoxMaterialCategory.Items.Clear()
        ...
        Material.ComboBoxVat.Items.Clear()
        Material.ComboBoxUseUnit.Items.Clear()
        Material.ComboBoxPOUnit.Items.Clear()
        Material.ComboBoxPOUnit2.Items.Clear()
        Material.ComboBoxStatus.Text = Nothing
        Material.ComboBoxStatus.Items.Clear()
        Material.PictureBoxResize.BackgroundImage = Nothing
        'LoadComboboxVat()
        'loadCombobox()
    End Sub
