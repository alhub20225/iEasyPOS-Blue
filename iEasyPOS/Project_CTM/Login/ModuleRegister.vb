...
Module ModuleRegister
    ...
    Sub AddRegister()
        RandomPassword()
        Dim FristName As String = Register.TxtFristName.Text
        Dim LastName As String = Register.TxtLastName.Text
        Dim BirthDay As String = Convert.ToDateTime(Register.DateTimeBirthDay.Value).ToString("yyyy-MM-dd", New System.Globalization.CultureInfo("th-TH"))
        ...
        Dim Registerdate As String = Convert.ToDateTime(Now).ToString("yyyy-MM-dd HH:mm:ss", New System.Globalization.CultureInfo("th-TH"))
        'Check index Phone
        ...
        If FristName = "" Or LastName = "" Or Email = "" Or Phone = "" Then
        ...
        Else
            ...
            Else
                ...
                field = "FristName, LastName, Passwords, BirthDay, Email, Phone, Registerdate, Active, UpdateDate, Deletedate, EditUserId, DeleteUserId, RoleId,Hash,GenderId,NationalityId"
                table = "Users"
                Dim value As String = "'" & FristName & "','" & LastName & "','" & PassWordRan & "','" & BirthDay & "','" & Email & "','" & Phone & "','" & Registerdate & "',1,'','','','',1,'" & RandomHashing() & "',1,1"
                ModuleGetDataAPI.gett("http://testnoi.hiveup.co/api/vbapi.php?action=INSERT&field=" & field & "&table=" & table & "&value=" & value & "")
                'executesql("INSERT INTO Users(FristName, LastName, Passwords, BirthDay, Email, Phone, Registerdate, Active, UpdateDate, Deletedate, EditUserId, DeleteUserId, RoleId,,Hash,GenderId,NationalityId) VALUES('" & FristName & "','" & LastName & "','" & PassWordRan & "','" & BirthDay & "','" & Email & "','" & Phone & "','" & Registerdate & "',1,'','','','',2,'" & RandomHashing() & "',1,1)")
    ...
    '    Dim Email As String = Register.TxtEmail.Text
    '    Dim Phone As String = Register.TxtPhone.Text
    '    '"INSERT INTO Users(FristName,LastName,BirthDay,Email,Phone,Active) VALUES('" & FristName & "','" & Lastname & "','" & BirthDay & "','" & Email & "','" & Phone & "',1)"
    '    ConnectServerDataBase("INSERT INTO users(FristName) VALUES('" & FristName & "')")
    '    Clipboard.SetText("INSERT INTO Users(FristName) VALUES('" & FristName & "')")
    'End Sub
End Module
