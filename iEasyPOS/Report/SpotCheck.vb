...
Public Class SpotCheck
    ...
    Dim DTS As String = Convert.ToDateTime(Now).ToString("yyyy-MM-ddT00:00:00", New System.Globalization.CultureInfo("en-US"))
    Dim DTE As String = Convert.ToDateTime(Now).ToString("yyyy-MM-ddT23:59:59", New System.Globalization.CultureInfo("en-US"))
    ...
    Private Sub SpotCheck_BeforePrint(sender As Object, e As PrintEventArgs) Handles Me.BeforePrint
        Try
            ...
            Dim Datatable2 As DataTable = executesql("SELECT SubTotal FROM Sale WHERE SaleDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126) AND Active=1")
...
(Select Sum(Sale.AmountDC) from Sale where Sale.SaleDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126) AND Sale.Active=1
) As AmountDC,
(Select Sum(Sale.TaxAmt) from Sale where Sale.SaleDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126) AND Sale.Active=1
) As Tax
FROM SaleItem inner join sale on(Sale.SaleId = SaleItem.SaleId) WHERE SaleItem.insertDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126) AND Sale.Active=1")
        ...
        Where Sale.Active = 1  And PaymentType.PaymentId = 15 And Sale.SaleDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126)")
            'TotalQrAlipay
        ...
        Where Sale.Active = 1  And PaymentType.PaymentId= 16 And Sale.SaleDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126)")
            'TotalQrWechat
        ...
        Where Sale.Active = 1  And PaymentType.PaymentId= 17 And Sale.SaleDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126)")
            'TotalCreditCrad
            Dim Datatable11 As DataTable = executesql("Select ISNULL(sum(PaymentDetail.PaidAmount),0) As SubTotal from PaymentDetail inner Join Sale On(Sale.SaleId = PaymentDetail.SaleId) inner Join PaymentType  On (PaymentType.PaymentId = PaymentDetail.PaymentTypeId) Where Sale.Active = 1  And PaymentType.PaymentId = 11 And Sale.SaleDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126)")

            Dim Datatable12 As DataTable = executesql("Select  (sum(Sale.SubTotal) - 
    ISNULL((Select sum(PaymentDetail.PaidAmount) from PaymentDetail inner Join Sale On(Sale.SaleId = PaymentDetail.SaleId) inner Join PaymentType  On (PaymentType.PaymentId = PaymentDetail.PaymentTypeId) Where Sale.Active = 1  And PaymentType.PaymentId = 11 And Sale.SaleDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126)),0)-
    ISNULL((Select sum(PaymentDetail.PaidAmount) from PaymentDetail inner Join Sale On(Sale.SaleId = PaymentDetail.SaleId) inner Join PaymentType  On (PaymentType.PaymentId = PaymentDetail.PaymentTypeId) Where Sale.Active = 1  And PaymentType.PaymentId = 17 And Sale.SaleDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126)),0)-
    ISNULL((Select sum(PaymentDetail.PaidAmount) from PaymentDetail inner Join Sale On(Sale.SaleId = PaymentDetail.SaleId) inner Join PaymentType  On (PaymentType.PaymentId = PaymentDetail.PaymentTypeId) Where Sale.Active = 1  And PaymentType.PaymentId = 16 And Sale.SaleDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126)),0)-
    ISNULL((Select sum(PaymentDetail.PaidAmount) from PaymentDetail inner Join Sale On(Sale.SaleId = PaymentDetail.SaleId) inner Join PaymentType  On (PaymentType.PaymentId = PaymentDetail.PaymentTypeId) Where Sale.Active = 1  And PaymentType.PaymentId = 15 And Sale.SaleDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126)),0)
    ) As SubTotal
        from  Sale 
    Where Sale.Active = 1  And Sale.SaleDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126)")
    ...
    Private Sub LabelTotalOrder_BeforePrint(sender As Object, e As PrintEventArgs) Handles LabelTotalOrder.BeforePrint
        ...
        Dim Datatable2 As DataTable = executesql("SELECT SUM(SubTotal) As SubTotal FROM Sale WHERE SaleDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126) AND Active=1")
    ...
    Private Sub LabelTotalDeleteSale_BeforePrint(sender As Object, e As PrintEventArgs) Handles LabelTotalDeleteSale.BeforePrint
        Dim DT As DataTable = executesql("select sum(Sale.SubTotal) as SubTotal  from Sale WHERE Sale.SaleDate BETWEEN CONVERT(datetime, '" & DTS & "', 126) AND CONVERT(datetime, '" & DTE & "', 126) AND Sale.Active = 2")
    ...
    Private Sub SpotCheck_DataSourceDemanded(sender As Object, e As EventArgs) Handles MyBase.DataSourceDemanded
