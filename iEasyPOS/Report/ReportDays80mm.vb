...
Public Class ReportDays80mm
    ...
    Dim Date_start As String = Convert.ToDateTime(Frm_ReportDay80mm.DTP1.Value).ToString("yyyy-MM-dd 00:00:00", New System.Globalization.CultureInfo("th-TH"))
    Dim Date_End As String = Convert.ToDateTime(Frm_ReportDay80mm.DTP1.Value).ToString("yyyy-MM-dd 23:59:59", New System.Globalization.CultureInfo("th-TH"))
...
Where Sale.Active = 1  And PaymentType.PaymentId= 17 And Sale.SaleDate BETWEEN '" & Date_start & "' AND '" & Date_End & "'")
    ...
    Private Sub ReportDays80mm_ParametersRequestBeforeShow(sender As Object, e As ParametersRequestEventArgs) Handles Me.ParametersRequestBeforeShow
        txtSaleDate.Text = Convert.ToDateTime(Parameter1.Value).ToString("dd/MM/yyyy HH:mm:ss", New System.Globalization.CultureInfo("th-TH"))
        'txtSaleDateEnd.Text = Convert.ToDateTime(Parameter2.Value).ToString("dd/MM/yyyy HH:mm:ss", New System.Globalization.CultureInfo("th-TH"))
    ...
    Private Sub XrLabel18_BeforePrint(sender As Object, e As PrintEventArgs) Handles XrLabel18.BeforePrint
        ...
        If IsDBNull(AmountDC(0)("AmountDC")) Then
            Exit Sub
        Else
            Dim SelectDC As DataTable = executesql("select SUM(SaleItem.PercentDC + SaleItem.BahtDC) AS DC from SaleItem inner join Sale on(Sale.SaleId = SaleItem.SaleId)  WHERE SaleItem.InsertDate BETWEEN  '" & Date_start & "' AND '" & Date_End & "' AND Sale.Active= 1")
    ...
    Private Sub XrLabel16_BeforePrint(sender As Object, e As PrintEventArgs) Handles XrLabel16.BeforePrint
        Dim sum_withDraw As DataTable = executesql("select sum(WithdrawAmount) as WithdrawAmount from WithdrawMoney where InsertDate = '" & Date_start & "' AND Active = 1")
