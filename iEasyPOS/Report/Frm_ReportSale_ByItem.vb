...
Public Class Frm_ReportSale_ByItem
    ...
    Private Sub Btn_Search_Click(sender As Object, e As EventArgs) Handles Btn_Search.Click
        ...
        Dim Datatable1 As DataTable = executesql("select Item.ItemName,SUM(SaleItem.Qty) As Qty,Item.ItemPrice,SUM(SaleItem.BahtDC + SaleItem.PercentDC) As Discount,SUM(SaleItem.SubTotal) As SubTotal,SUM(SaleItem.Tax) As Tax from Sale inner join SaleItem on(Sale.SaleId = SaleItem.SaleId) inner join Item on(SaleItem.ItemId = Item.ItemId) inner join PaymentDetail on(PaymentDetail.SaleId = Sale.SaleId) inner join CategoryItem on(CategoryItem.CategoryId = Item.CategoryId) Where SaleDate BETWEEN CONVERT(datetime, '" & Date_start & "', 126) AND CONVERT(datetime, '" & Date_End & "', 126) and Sale.Active = 1 and CategoryItem.CategoryName = '" & CateName & "' Group by Item.ItemName,Item.ItemId,Item.ItemPrice Order by Item.ItemId ASC")
