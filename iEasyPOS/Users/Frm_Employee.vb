...
Public Class Frm_Employee
    ...
    Private Sub Frm_Employee_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        DataGridViewShowUser.RowTemplate.Height = 35
        DateTimePicker_BirthDate.EditValue = Now
        DateTimePicker_WorkStart.EditValue = Now
        loadData()
        loadUser()
    End Sub
    Sub loadUser()
        ...
        Dim selectUser As DataTable = executesql("SELECT Users.UserId,Users.FirstName,Roles.RoleName FROM Users INNER JOIN Roles ON Roles.RoleId = Users.RoleId  WHERE Users.Active=1")
        For i As Integer = 0 To selectUser.Rows.Count - 1
            DataGridViewShowUser.Rows.Add(selectUser(i)("UserId"), selectUser(i)("FirstName"), selectUser(i)("RoleName"))
        Next
        userId = 0
    End Sub
    Sub loadData()
        ...
        Else
            For i As Integer = 0 To selectGender.Rows.Count - 1
                ComboBoxGenger.Properties.Items.Add(selectGender(i)("GenderName"))
            Next
        ...
        Else
            For i As Integer = 0 To selectNationality.Rows.Count - 1
                ComboBoxNationality.Properties.Items.Add(selectNationality(i)("NationalityNameTH"))
            Next
        ...
        Else
            For i As Integer = 0 To selectRole.Rows.Count - 1
                ComboBoxRole.Properties.Items.Add(selectRole(i)("RoleName"))
            Next
        ...
        Else
            ...
            For i As Integer = 0 To selectPayType.Rows.Count - 1
                ComboBoxUnit.Properties.Items.Add(selectPayType(i)("PayTypeNameTH"))
            Next
        ...
        Else
            For i As Integer = 0 To selectOverTimePayType.Rows.Count - 1
                ComboBoxUnit2.Properties.Items.Add(selectOverTimePayType(i)("PayTypeNameTH"))
            Next
        ...
        Else
            For i As Integer = 0 To selectDiligenceType.Rows.Count - 1
                ComboBoxUnit3.Properties.Items.Add(selectDiligenceType(i)("PayTypeNameTH"))
            Next
        ...
        Else
            For i As Integer = 0 To selectBreak.Rows.Count - 1
                ComboBoxBreak.Properties.Items.Add(selectBreak(i)("BreakName"))
            Next
        ...
        For i As Integer = 0 To selectDay.Rows.Count - 1
            ...
            ShowDay.Font = New System.Drawing.Font("Kanit", 12, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, CType(222, Byte))
        ...
        Else
            For i As Integer = 0 To selectWorkType.Rows.Count - 1
                ComboBoxWorkType.Properties.Items.Add(selectWorkType(i)("WorkTypeName"))
            Next
    ...
    Sub clearText()
        ...
        txtDiligence.Clear()
        txtEmail.Clear()
        DateTimePicker_BirthDate.EditValue = Date.Today()
        Dim Chk As CheckBox
    ...
    Private Sub DataGridViewShowUser_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridViewShowUser.CellClick
        ...
        Else
            ...
            DateTimePicker_BirthDate.EditValue = birthDay
        End If
        Dim workStartDate As DateTime
        If IsDBNull(selecUserFromDataGridView(0)("WorkStartDate")) Then
            DateTimePicker_WorkStart.EditValue = Date.Today()
        Else
            workStartDate = Convert.ToDateTime(selecUserFromDataGridView(0)("WorkStartDate"), _cultureEnInfo)
            DateTimePicker_WorkStart.EditValue = workStartDate
        End If
        txtWorkStartDate.Text = 543 - DateDiff(DateInterval.Year, CDate(Date.Now), CDate(workStartDate)) & "ปี"

        'คำนวณวัน
        Dim dayThisYear As New Date(Date.Today.Year, Me.DateTimePicker_WorkStart.EditValue.Month, Me.DateTimePicker_WorkStart.EditValue.Day)
        Dim years = Date.Today.Year - Me.DateTimePicker_WorkStart.EditValue.Year
        Dim months = Date.Today.Month - Me.DateTimePicker_WorkStart.EditValue.Month
        Dim days = Date.Today.Day - Me.DateTimePicker_WorkStart.EditValue.Day
    ...
    Private Sub btnAddUser_Click(sender As Object, e As EventArgs) Handles btnAddUser.Click
        ...
        If userId = 0 Then
            ...
            End If
        Else
            Dim birthDay As String = Convert.ToDateTime(DateTimePicker_BirthDate.Text).ToString("yyyy-MM-dd", _cultureEnInfo)
            Dim workTime As String = Convert.ToDateTime(DateTimePicker_WorkStart.Text).ToString("yyyy-MM-dd", _cultureEnInfo)
            Dim DateNow As String = Convert.ToDateTime(Now).ToString("yyyy-MM-dd HH:mm:ss", _cultureEnInfo)

            executesql("UPDATE Users SET FirstName='" & txtEmployeeFullName.Text & "',LastName='" & txtEmployeeLastName.Text & "',NickName='" & txtNickName.Text & "',BirthDay='" & birthDay & "',Email='" & txtEmail.Text & "',CardId='" & txtCardID.Text & "',GenderId='" & ComboBoxGenger.Text & "',NationalityId='" & ComboBoxNationality.Text & "',Address='" & txtAddress.Text & "',Phone='" & txtPhone1.Text & "',Phone2='" & txtPhone2.Text & "',Phone3='" & txtPhone3.Text & "',RoleId='" & ComboBoxRole.Text & "',WorkTypeId='" & ComboBoxWorkType.Text & "',WorkStartDate='" & workTime & "',Pay=" & txtPay.Text & ",PayTypeId=" & ComboBoxUnit.Text & ",OverTimePay=" & txtOverTimePay.Text & ",OverTimePayTypeId=" & ComboBoxUnit2.Text & ",Diligence=" & txtDiligence.Text & ",DiligenceTypeId=" & ComboBoxUnit3.Text & ",BreakId=" & ComboBoxBreak.Text & ",UpdateDate='" & DateNow & "' WHERE UserId='" & userId & "'")
            ...
            For Each CheckBoxUpdate In ShowDayOfWeek.Controls
                ...
                End If
            Next
            'If PictureUsersResize.BackgroundImage = Nothing Then
            If imageUser <> "" Then
    ...
    Public Sub txtPhone3_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs)
        ...
        End Select
    End Sub
    Private Sub txtPay_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs)
        Select Case Asc(e.KeyChar)
            Case 48 To 57 ' key โค๊ด ของตัวเลขจะอยู่ระหว่าง48-57ครับ 48คือเลข0 57คือเลข9ตามลำดับ
                e.Handled = False
            Case 8, 13, 46 ' ปุ่ม Backspace = 8,ปุ่ม Enter = 13, ปุ่มDelete = 46
    ...
    Private Sub btnClearImage_Click(sender As Object, e As EventArgs) Handles btnClearImage.Click
        'BrowsImageUsers.Controls.Clear()
        PictureUsersResize.BackgroundImage = iEasyPOS.My.Resources.Resources.Pic51
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        Using plexi = New Form()
            ...
            plexi.Bounds = Screen.FromPoint(Frm_Break.Location).Bounds
            ...
            plexi.Opacity = 0.6
            plexi.Show()
            Frm_Break.StartPosition = FormStartPosition.CenterParent
            Frm_Break.ShowDialog(plexi)
        End Using
        'Frm_Break.Show()
    End Sub
