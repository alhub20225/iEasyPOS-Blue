Module ModulePosTable
    ...
    Sub TableZoneName()
        ...
        'End If
    End Sub
    Dim ItemColumn As Integer = My.Settings.ItemColumn
    Sub LoadCategory()
    ...
    Sub AddPOSItem_(sender As Object, e As EventArgs)
        ...
        Dim TB_saleName As String = TableName
    ...
    Sub calculatePrice()
        TB_Total = 0
        totalDiscount = 0
        POS.txtDiscount.Text = FormatNumber(POS.txtDiscount.Text, 2)
    ...
    Sub SaveSaleTable()
        ...
        Dim TB_TableId As Integer = 0
        Dim LoadDatatable As DataTable = executesql("SELECT TableId,TableName,TableId2,TableName2 FROM Tables WHERE TableId2='" & ModuleZoneTable.TableId2 & "'")
        If LoadDatatable.Rows.Count = 0 Then
            TB_saleName = ModuleZoneTable.TableName
            TB_TableId = ModuleZoneTable.TableId
        Else
            TB_saleName = ModuleZoneTable.TableName2
            TB_TableId = ModuleZoneTable.TableId2
        End If
        'MsgBox(TB_saleName)
        'Exit Sub
        Dim TB_InsertDate As String = Convert.ToDateTime(Now).ToString("yyyy-MM-dd HH:mm:ss", New System.Globalization.CultureInfo("en-US"))
        ...
        'Dim TB_TaxItem As Decimal
        'Dim TB_BahtDC As Decimal
        Dim SaleDate As String = Convert.ToDateTime(Now).ToString("yyyy-MM-dd HH:mm:ss", New System.Globalization.CultureInfo("en-US"))
        ...
        Dim StartTime As String = Convert.ToDateTime(Now).ToString("yyyy-MM-dd HH:mm:ss", New System.Globalization.CultureInfo("en-US"))
        Dim EndTime As String = Convert.ToDateTime(Now).ToString("yyyy-MM-dd HH:mm:ss", New System.Globalization.CultureInfo("en-US"))
        Dim UserId As Integer = Frm_Login.UserId
        ...
        If TB_GetSaleId = 0 Then
            'Bill 1
            Dim bill_id As Integer = 1
            Dim bill_name As String = "บิลที่ 1"
            ...
            executesql("INSERT INTO SaleTable(SaleTabId,SaleId,SaleName,InsertDate,OpenUserId,Active,CurrentUserUsed,BillSplit_Id,BillSplit_Name,TableId) VALUES('" & selectSaleTabId(0)("SaleTabId") & "','" & selectSaleId(0)("SaleId") & "','" & TB_saleName & "','" & TB_InsertDate & "','" & TB_openUserId & "','" & TB_Active & "','" & Frm_Login.UserId & "','" & bill_id & "', '" & bill_name & "','" & TB_TableId & "')")
            ' อัพเดทโต๊ะ
            ...
            For i As Integer = 0 To POS.DataGridViewPOS.RowCount - 1
                ...
                If POS.DataGridViewPOS.Rows(i).Cells("PromotionType").Value Is Nothing Or POS.DataGridViewPOS.Rows(i).Cells("ส่วนลด").Value <> 0 Then
                    executesql("INSERT INTO SaleItem(SaleId,ItemId,CategoryId,ItemCode,ItemName,InitPrice,UnitPrice,Qty,SubTotal,Tax,Total,InsertDate,PercentDC,BahtDC,AmtDC,TerminalName,Active,StatusPrint,PrinterName,UserId,BillSplit_Id,BillSplit_Name) VALUES('" & selectSaleId(0)("SaleId") & "','" & ItemId & "','" & SelectDataItem(0)("CategoryId") & "','" & SelectDataItem(0)("ItemCode") & "','" & POS.DataGridViewPOS.Rows(i).Cells(0).Value & "','" & InitPrice & "','" & UnitPrice & "','" & Qty & "','" & SubTotalItem & "','" & TaxAmount & "','" & TotalItem & "','" & InsertDate & "','" & PercentDC & "','" & BahtDC & "','" & AmtDC & "','" & TerminalName & "',3,0,'" & Printer_cate(0)("CatePrinterName") & "','" & Frm_Login.UserId & "','" & bill_id & "','" & bill_name & "')")
                    ...
                    Dim GetPrinterName As DataTable = executesql("SELECT CatePrinterName FROM CategoryItem WHERE CategoryId='" & GetSaleItemID(0)("CategoryId") & "'")
                    executesql("INSERT INTO SaleItemOrder(SaleItemId,Qty,UserId,OrderTime,PrinterName,TerminalName,StatusPrint,ItemName,Active) VALUES('" & GetSaleItemID(0)("SaleItemId") & "'," & Qty & ",'" & Frm_Login.UserId & "','" & Convert.ToDateTime(GetSaleItemID(0)("InsertDate")).ToString("yyyy-MM-dd HH:mm:ss", New System.Globalization.CultureInfo("en-US")) & "','" & GetPrinterName(0)("CatePrinterName") & "','" & Environment.MachineName & "'," & AllowPrint & ",'" & GetSaleItemID(0)("ItemName") & "',1)")
                End If
            Next
        ...
        Else
            ...
            AmountDC = POS.txtDiscount.Text
            'End If
            executesql("UPDATE SaleTable SET UpdateDate='" & TB_UpdateDate & "',SaleName='" & TB_saleName & "' WHERE SaleId='" & TB_GetSaleId & "'")
            ...
            Dim select_bill As DataTable = executesql("SELECT BillSplit_Id,BillSplit_Name From Sale where SaleId ='" & TB_GetSaleId & "'")
            Dim bill_id As Integer = select_bill(0)("BillSplit_Id")
            Dim bill_name As String = select_bill(0)("BillSplit_Name")
            ...
            For i As Integer = 0 To POS.DataGridViewPOS.Rows.Count - 1
                ...
                UnitPrice = FormatNumber(POS.DataGridViewPOS.Rows(i).Cells(2).Value, 2)
                TotalItem = POS.DataGridViewPOS.Rows(i).Cells(2).Value * Qty
                SubTotalItem = (((POS.DataGridViewPOS.Rows(i).Cells(2).Value * Qty)) - PercentDC) - BahtDC
                'MsgBox(SubTotalItem)
                'FormatNumber(InitPrice * Qty, 2)
                'TaxAmount = ((((SelectDataItem(0)("ItemPrice") * Qty) - PercentDC) - BahtDC) - ((((SelectDataItem(0)("ItemPrice") * Qty) - PercentDC) - BahtDC) * 100 / (SelectDataItem(0)("Multiply") + 100)))
                TaxAmount = ((((SelectDataItem(0)("ItemPrice") * Qty))) - ((((SelectDataItem(0)("ItemPrice") * Qty))) * 100 / (SelectDataItem(0)("Multiply") + 100)))
                TaxAmt += TaxAmount
                ...
                If POS.DataGridViewPOS.Rows(i).Cells("PromotionType").Value Is Nothing Or POS.DataGridViewPOS.Rows(i).Cells("ส่วนลด").Value <> 0 Then
                    ...
                    If POS.DataGridViewPOS.Rows(i).Cells("SaleItemId").Value IsNot Nothing Then
                        ...
                        If checkItem(i)("Qty") > POS.DataGridViewPOS.Rows(i).Cells(1).Value Then
                            ...
                            executesql("insert into CancelSaleItem(SaleItemId,ItemCode,ItemName,Qty,InitPrice,UnitPrice,Total,SubTotal,Tax,TableId,TerminalName,AllowPrint,PrinterName,StatusPrint,Active,BillSplit_Id,BillSplit_Name) values('" & checkItem(i)("SaleItemId") & "','" & SelectDataItem(0)("ItemCode") & "','" & SelectDataItem(0)("ItemName") & "','" & checkItem(i)("Qty") - POS.DataGridViewPOS.Rows(i).Cells(1).Value & "','" & InitPrice & "','" & UnitPrice & "','" & TotalItem & "','" & SubTotalItem & "','" & TaxAmount & "','" & TableId & "','" & Environment.MachineName & "',1,'" & printerName(0)("CatePrinterName") & "',0,1,'" & bill_id & "','" & bill_name & "')")

                        ElseIf checkItem(i)("Qty") < POS.DataGridViewPOS.Rows(i).Cells(1).Value Then
                            Dim new_qty As Integer = POS.DataGridViewPOS.Rows(i).Cells(1).Value - checkItem(i)("Qty")
                            'MsgBox("New_qty = " & new_qty)
                            executesql("INSERT INTO SaleItemOrder(SaleItemId,Qty,UserId,OrderTime,PrinterName,TerminalName,StatusPrint,ItemName,Active) VALUES('" & checkItem(i)("SaleItemId") & "'," & new_qty & ",'" & Frm_Login.UserId & "','" & UpdateDate & "','" & printerName(0)("CatePrinterName") & "','" & Environment.MachineName & "'," & AllowPrint & ",'" & SelectDataItem(0)("ItemName") & "','1')")
                    ...
                    Else
                        ...
                        checkTax = True
                        If POS.DataGridViewPOS.Rows(i).Cells("PromotionType").Value Is Nothing Or POS.DataGridViewPOS.Rows(i).Cells("ส่วนลด").Value <> 0 Then
                            executesql("INSERT INTO SaleItem(SaleId,ItemId,CategoryId,ItemCode,ItemName,InitPrice,UnitPrice,Qty,SubTotal,Tax,Total,InsertDate,PercentDC,BahtDC,AmtDC,TerminalName,Active,StatusPrint,PrinterName,UserId,BillSplit_Id,BillSplit_Name) VALUES('" & TB_GetSaleId & "','" & ItemId & "','" & SelectDataItem(0)("CategoryId") & "','" & SelectDataItem(0)("ItemCode") & "','" & POS.DataGridViewPOS.Rows(i).Cells(0).Value & "','" & InitPrice & "','" & UnitPrice & "','" & Qty & "','" & SubTotalItem & "','" & TaxAmount & "','" & TotalItem & "','" & InsertDate & "','" & PercentDC & "','" & BahtDC & "','" & AmtDC & "','" & TerminalName & "',3,0,'" & Printer_cate(0)("CatePrinterName") & "','" & Frm_Login.UserId & "','" & bill_id & "','" & bill_name & "')")

                            Dim selectSaleItem As DataTable = executesql("SELECT Max(SaleItemId) As SaleItemId FROM SaleItem")
                            Dim SaleItemId As Integer = selectSaleItem(0)("SaleItemId")

                            executesql("INSERT INTO SaleItemOrder(SaleItemId,Qty,UserId,OrderTime,PrinterName,TerminalName,StatusPrint,ItemName,Active) VALUES('" & SaleItemId & "'," & Qty & ",'" & Frm_Login.UserId & "','" & UpdateDate & "','" & Printer_cate(0)("CatePrinterName") & "','" & Environment.MachineName & "'," & AllowPrint & ",'" & POS.DataGridViewPOS.Rows(i).Cells(0).Value & "','1')")
        ...
        End If
        ModulePrinrOrder.PrinteOrder(TB_GetSaleId)
    ...
    Sub LoadSaveSaleTable()
        ...
        Else
            ...
            Else
                checkSplitBill = executesql("SELECT Sale.SaleId FROM Sale 
                                            ...
                                            INNER JOIN SaleItem on (SaleItem.SaleId = Sale.SaleId)
                                            INNER JOIN Tables on (Tables.TableId = SaleTabRef.TableId) 
                                            WHERE SaleTabRef.TableId in(" & TableId2(0)("TableId2") & ") AND Sale.Active=3 AND SaleItem.Active=3 AND               Tables.IsSplitBill=1
                                            GROUP BY Sale.SaleId")
        ...
        For i As Integer = 0 To selectSaleData.Rows.Count - 1
            ...
            If selectSaleData(i)("AmountDC") <> 0 Then
                ...
                Type = 1
            End If
            Dim SelectUseTopping As DataTable = executesql("select                                                         UseTopping.ToppingId,UseTopping.SaleItemId,UseTopping.ToppingName,UseTopping.InsertDate,
                                                            SaleItem.ItemId
                                                            from UseTopping
        ...
        POS.BtnCheck.Visible = True

        Dim dtCustomer As DataTable = executesql("SELECT Sale.CustomerId, Customer.FirstName, Customer.LastName FROM Sale INNER JOIN Customer ON Sale.CustomerId = Customer.CustomerId where Sale.SaleId=" & TB_GetSaleId & "")
        If dtCustomer.Rows.Count > 0 Then
            POS.BtnPoint.Enabled = True
            POS.PanelMember.Visible = True
            POS.LabelMemberName.Text = "Customer : " & dtCustomer(0)("FirstName")
        End If
    ...
    End Sub
    Sub DelItem()
        If MsgBox("ต้องการยกเลิกรายการหรือไม่", MsgBoxStyle.OkCancel, "ยกเลิกการขาย") = MsgBoxResult.Ok Then
            ...
            PrintCancleOrder(TB_GetSaleId, SaleItemId)

            If POS.DataGridViewPOS.Rows.Count = 0 Then ModulePosTable.CancelBill()
        Else
            Exit Sub
        End If
    ...
    Sub CancelBill()
        ...
        If POS.DataGridViewPOS.Rows.Count = 0 Then
            ' คืนค่าเดิมโต๊ะ
            executesql("update Tables set StateStatus= 0,IsGroupTable = 0, IsSplitBill = 0 ,TableName2= '', TableId2 = 0 where TableId = '" & ModuleZoneTable.TableId & "'")
            GetSaleId = 0
        ...
        Else
            ...
            'MsgBox("GetSaleId = " & GetSaleId)
            'Exit Sub
            If MsgBox("ต้องการยกเลิกการขายหรือไม่", MsgBoxStyle.OkCancel, "ยกเลิกการขาย") = MsgBoxResult.Ok Then
                Dim LoadDatatable As DataTable = executesql("SELECT TableId,TableName,TableId2,TableName2 FROM Tables WHERE TableId2='" & ModuleZoneTable.TableId2 & "'")
                Dim SaleItem As DataTable = executesql("select * from SaleItem where SaleId = '" & TB_GetSaleId & "'")
                For i As Integer = 0 To SaleItem.Rows.Count - 1
                    ...
                    End If
                    Dim printerName As DataTable = executesql("SELECT CatePrinterName FROM CategoryItem WHERE CategoryId='" & SelectDataItem(0)("CategoryId") & "'")
                    Dim Bill As DataTable = executesql("select BillSplit_Id,BillSplit_Name from SaleTable where SaleId = '" & TB_GetSaleId & "'")
                    Dim Bill_id As Integer = Bill(0)("BillSplit_Id")
                    Dim Bill_Name As String = Bill(0)("BillSplit_Name")

                    executesql("insert into CancelSaleItem(SaleItemId,ItemCode,ItemName,Qty,InitPrice,UnitPrice,Total,SubTotal,Tax,TableId,TerminalName,AllowPrint,PrinterName,StatusPrint,Active,BillSplit_Id,BillSplit_Name) values('" & SaleItem(i)("SaleItemId") & "','" & SelectDataItem(0)("ItemCode") & "','" & SelectDataItem(0)("ItemName") & "','" & Qty & "','" & SaleItem(i)("InitPrice") & "','" & SaleItem(i)("UnitPrice") & "','" & SaleItem(i)("Total") & "','" & SaleItem(i)("SubTotal") & "','" & SaleItem(i)("Tax") & "','" & ModuleZoneTable.TableId & "','" & Environment.MachineName & "',1,'" & printerName(0)("CatePrinterName") & "',0,1,'" & Bill_id & "','" & Bill_Name & "')")
                Next
                If LoadDatatable.Rows.Count = 0 Then
                    'update ยกเลิกบิล >>> Active=4
                    executesql("update SaleItem set Active=4,UpdateDate='" & UpdateDate & "' where SaleId='" & TB_GetSaleId & "' and Active=3")
                    executesql("update Sale set Active=4,UpdateDate='" & UpdateDate & "' where SaleId='" & TB_GetSaleId & "' and Active=3")
                Else
                    'update ยกเลิกบิล(รวมโต๊ะ) >>> Active=5
                    executesql("update SaleItem set Active=5,UpdateDate='" & UpdateDate & "' where SaleId='" & TB_GetSaleId & "' and Active=3")
                    executesql("update Sale set Active=5,UpdateDate='" & UpdateDate & "' where SaleId='" & TB_GetSaleId & "' and Active=3")
                End If
                'update SaleTable
                executesql("update SaleTable set Active=0 where SaleId='" & TB_GetSaleId & "' AND Active=1")
                'เช็คสถานะแยกบิล
                Dim checkRowsSaleTable As DataTable = executesql("select BillSplit_Id,BillSplit_Name from SaleTable where SaleName = '" & ModuleZoneTable.TableName & "' and Active = 1")
                If checkRowsSaleTable.Rows.Count = 0 Then
                    If LoadDatatable.Rows.Count = 0 Then 'กรณีไม่รวมโต๊ะ
                        executesql("update Tables set StateStatus= 0,IsGroupTable = 0, IsSplitBill = 0 ,TableName2= '', TableId2 = 0 where TableId = '" & ModuleZoneTable.TableId & "'")
                    Else 'กรณีรวมโต๊ะ
                        executesql("update Tables set StateStatus= 0,IsGroupTable = 0, IsSplitBill = 0 ,TableName2= '', TableId2 = 0 where TableId2 = '" & ModuleZoneTable.TableId & "'")
                    End If
                End If
                If LoadDatatable.Rows.Count = 0 Then
                    'ปลิ้นใบเสร็จยกเลิกโต๊ะ กรณีไม่รวมโต๊ะ
                    ModulePrinrOrder.PrintCancleBill(TB_GetSaleId)
                Else
                    'ปลิ้นใบเสร็จยกเลิกโต๊ะ กรณีรวมโต๊ะ
                    ModulePrinrOrder.PrintCancleBill2(TB_GetSaleId)
                End If
