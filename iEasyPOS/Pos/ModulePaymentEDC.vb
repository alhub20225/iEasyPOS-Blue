Module ModulePaymentEDC
    Dim Balance As Decimal
    Sub EDCAddSale(ByVal PaymentType As Integer, ByVal Subtotal As Decimal)
        ...
        Else
            ...
            Dim checkSaleNo2 As DataTable = executesql("SELECT SaleNo FROM Sale WHERE SaleDate LIKE '%" & Convert.ToDateTime(Now).ToString("yyyy-MM-dd", New System.Globalization.CultureInfo("en-US")) & "%' And Active = 1")
            'Dim checkSaleNo As DataTable = executesql("SELECT SaleNo FROM Sale WHERE SaleDate='" & InsertDate & "'")
            No = 0
